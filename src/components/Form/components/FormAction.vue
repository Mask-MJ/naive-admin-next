<template>
  <n-gi v-bind="actionGiOptions" v-if="showActionButtonGroup">
    <n-form-item-gi>
      ...
      <n-button quaternary size="small" @click="toggleAdvanced" v-if="showAdvancedButton">
        {{ collapsed ? t('component.form.putAway') : t('component.form.unfold') }}
        <template #icon>
          <n-icon>
            <cash-icon />
...
<script setup lang="ts" name="BasicFormAction">
  ...
  const getResetBtnOptions = computed(() =>
    Object.assign({ label: t('common.resetText') }, props.resetButtonOptions),
  );
  const getSubmitBtnOptions = computed(() =>
    Object.assign({ label: t('common.queryText') }, props.submitButtonOptions),
  );
