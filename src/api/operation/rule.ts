import type { RuleList } from './types/rule';
...
/** 获取列表 */
export const getRuleList = (params) => defHttp.get<RuleList[]>({ url: Api.List, params });
/** 新增 */
export const addManage = (params) => defHttp.post({ url: Api.Manage, params });
/** 修改 */
export const setManage = (params) => defHttp.put({ url: Api.Manage, params });
/** 立即下发 */
...
/** 获取历史列表 */
export const getHistoryList = (params) => defHttp.get({ url: Api.HistoryList, params });
/** 审核 */
// export const auditRule = (params: AuditParams) => defHttp.put({ url: Api.Audit, params });
export const auditRule = (params) =>
  defHttp.put({ url: `${Api.Audit}?id=${params.id}&status=${params.status}` });
/** 详情 */
